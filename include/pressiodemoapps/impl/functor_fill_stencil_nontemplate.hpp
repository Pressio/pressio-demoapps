...
#ifndef PRESSIODEMOAPPS_STENCIL_FILLER_NONTEMPLATE_HPP_
#define PRESSIODEMOAPPS_STENCIL_FILLER_NONTEMPLATE_HPP_
...
template<
  ...
  class GhostDataType
  >
class StencilFillerNonTemplate;
...
public:
  StencilFillerNonTemplate() = delete;
  StencilFillerNonTemplate(const int stencilSize,
  ...
  void operator()(const index_t smPt, int ghostRow, int numDofPerCell)
  {
    if (numDofPerCell == 1){
      ...
      throw std::runtime_error("invalid numDofPerCell");
    }
  }
