...
class CrossShock2dGhostFiller
{

  using scalar_type = typename mesh_t::scalar_t;

public:
  CrossShock2dGhostFiller() = delete;
  CrossShock2dGhostFiller(const int stencilSize,
			  const state_t & stateIn,
			  const scalar_type gamma,
			  const scalar_type density,
			  const scalar_type inletXVel,
			  const scalar_type bottomYVel,
			  const mesh_t & meshIn,
			  ghost_t & ghostLeft,
			  ghost_t & ghostFront,
			  ghost_t & ghostRight,
  ...
  void operator()(index_t smPt, int gRow)
  {
	...
	m_ghostBack(gRow, 1) = m_density*m_inletXVel;
	m_ghostBack(gRow, 2) = 0.;
	m_ghostBack(gRow, 3) = m_state(uIndex+3);
      }
      else{
	m_ghostBack(gRow, 0) = m_state(uIndex);
	m_ghostBack(gRow, 1) = m_density*m_inletXVel;
	m_ghostBack(gRow, 2) = m_density*m_bottomYVel;
	m_ghostBack(gRow, 3) = m_state(uIndex+3);
      }
    }
	...
	if (myX < 0.5){
	  m_ghostBack(gRow, 4) = m_density;
	  m_ghostBack(gRow, 5) = m_density*m_inletXVel;
	  m_ghostBack(gRow, 6) = 0.;
	  ...
	  m_ghostBack(gRow, 4) = m_density;
	  m_ghostBack(gRow, 5) = m_density*m_inletXVel;
	  m_ghostBack(gRow, 6) = m_density*m_bottomYVel;
	  m_ghostBack(gRow, 7) = m_state(ind+3);
	}
      }
    }
	...
	if (myX < 0.5){
	  m_ghostBack(gRow, 8) = m_density;
	  m_ghostBack(gRow, 9) = m_density*m_inletXVel;
	  m_ghostBack(gRow, 10) = 0.;
	  ...
	  m_ghostBack(gRow, 8) = m_density;
	  m_ghostBack(gRow, 9) = m_density*m_inletXVel;
	  m_ghostBack(gRow, 10) = m_density*m_bottomYVel;
	  m_ghostBack(gRow, 11) = m_state(ind+3);
	}
      }
    }
