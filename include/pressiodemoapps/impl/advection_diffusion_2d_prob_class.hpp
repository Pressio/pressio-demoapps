...
public:
  ...
  state_type initialCondition() const
  {
    ...
    switch(m_probEn){
    case ::pressiodemoapps::AdvectionDiffusion2d::Burgers:{
...
protected:
  void initializeJacobian(jacobian_type & J)
  {
    ...
    // we currently only have inner cells because burgers is periodic
...
private:
			       ...
			       edge_rec_type & uPlusHalfPos) const
  {
    ...
    // for now, since we have Burgers, we only worry about inner cells
...
#pragma omp for schedule(static)
    ...
    for (decltype(graphRows.size()) it=0; it<graphRows.size(); ++it)
    {
      ...
      if (m_probEn == ::pressiodemoapps::AdvectionDiffusion2d::Burgers){
      	addBurgersDiffusionAndSourceToVelocityAndJacobian(U, V, J, smPt);
      }
    }
  }
...
#pragma omp for schedule(static)
    ...
    for (decltype(graphRows.size()) it=0; it<graphRows.size(); ++it){
      ...
      if (m_probEn == ::pressiodemoapps::AdvectionDiffusion2d::Burgers){
	addBurgersDiffusionAndSourceToVelocity(U, V, smPt);
      }
    }
  }

  template<class U_t, class V_t>
  void addBurgersDiffusionAndSourceToVelocity(const U_t & U,
						 V_t & V,
						 index_t smPt) const
  {
    ...
    V(smPt) += diffDxInvSq*( U(uIndexRight) - two*U(uIndex) + U(uIndexLeft) );
    V(smPt) += diffDyInvSq*( U(uIndexFront) - two*U(uIndex) + U(uIndexBack) );
    V(smPt) += 0.25;
  }

  template<class U_t, class V_t>
  void addBurgersDiffusionAndSourceToVelocityAndJacobian(const U_t & U,
							 V_t & V,
							 jacobian_type & J,
							 index_t smPt) const
  {
    ...
    J.coeffRef(smPt, uIndex) += -two*diffDxInvSq - two*diffDyInvSq;
