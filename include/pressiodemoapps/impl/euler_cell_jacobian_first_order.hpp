...
template<
  ...
  class MeshType
  >
class FirstOrderNearBDCellJacobianFunctor
{
  int m_axis = 1;
  SparseMatrix & m_J;
  const MeshType & m_meshObj;
...
public:
  FirstOrderNearBDCellJacobianFunctor(SparseMatrix & J,
				      const MeshType & meshObj,
  ...
  template<class index_t>
  void operator()(const index_t smPt, bool is_reflective = false)
  {
    const auto & graph = m_meshObj.graph();
    index_t l0 = {};
    index_t r0 = {};
    ...
    std::array<ScalarType, numDofPerCell> factors = { static_cast<ScalarType>(1) };
    if (numDofPerCell == 3 and is_reflective){
      factors[1] = static_cast<ScalarType>(-1);
    }
    else if (numDofPerCell == 4 and is_reflective){
      if (m_axis==1){
	factors[1] = static_cast<ScalarType>(-1);
      }
      else{
	factors[2] = static_cast<ScalarType>(-1);
      }
    }
    else if (numDofPerCell == 5 and is_reflective){
      if (m_axis==1){
	factors[1] = static_cast<ScalarType>(-1);
      }
      else if (m_axis==2){
	...
	factors[3] = static_cast<ScalarType>(-1);
      }
    }
	...
	for (int j=0; j<numDofPerCell; ++j)
	{
	  m_J.coeffRef(rowIndex+k, col_i+j)   = (factors[j]*m_JLneg(k,j) + m_JLpos(k,j) - m_JRneg(k,j))*m_hInv;
	  m_J.coeffRef(rowIndex+k, col_ip1+j) = -m_JRpos(k,j)*m_hInv;
	}
      }

      // ScalarType coeff = (is_reflective ) ? -1 : 1;
	...
	{
	  m_J.coeffRef(rowIndex+k, col_im1+j) = m_JLneg(k,j)*m_hInv;
	  m_J.coeffRef(rowIndex+k, col_i+j)   = (factors[j]*-m_JRpos(k,j) + m_JLpos(k,j) - m_JRneg(k,j))*m_hInv;
	}
      }

      // ScalarType coeff = (is_reflective ) ? -1 : 1;
      // m_J.coeffRef(rowIndex,  col_im1)   = m_JLneg(0,0)*m_hInv;
      // m_J.coeffRef(rowIndex,  col_im1+1) = m_JLneg(0,1)*m_hInv;
      // m_J.coeffRef(rowIndex,  col_im1+2) = m_JLneg(0,2)*m_hInv;
      // m_J.coeffRef(rowIndex,  col_i)     = (      -m_JRpos(0,0) + m_JLpos(0,0) - m_JRneg(0,0))*m_hInv;
      // m_J.coeffRef(rowIndex,  col_i+1)   = (coeff*-m_JRpos(0,1) + m_JLpos(0,1) - m_JRneg(0,1))*m_hInv;
      // m_J.coeffRef(rowIndex,  col_i+2)   = (      -m_JRpos(0,2) + m_JLpos(0,2) - m_JRneg(0,2))*m_hInv;

      // m_J.coeffRef(rowIndex+1, col_im1)   = m_JLneg(1,0)*m_hInv;
      // m_J.coeffRef(rowIndex+1, col_im1+1) = m_JLneg(1,1)*m_hInv;
      // m_J.coeffRef(rowIndex+1, col_im1+2) = m_JLneg(1,2)*m_hInv;
      // m_J.coeffRef(rowIndex+1, col_i)     = (      -m_JRpos(1,0) + m_JLpos(1,0)-m_JRneg(1,0))*m_hInv;
      // m_J.coeffRef(rowIndex+1, col_i+1)   = (coeff*-m_JRpos(1,1) + m_JLpos(1,1)-m_JRneg(1,1))*m_hInv;
      // m_J.coeffRef(rowIndex+1, col_i+2)   = (      -m_JRpos(1,2) + m_JLpos(1,2)-m_JRneg(1,2))*m_hInv;

      // m_J.coeffRef(rowIndex+2, col_im1)   = m_JLneg(2,0)*m_hInv;
      // m_J.coeffRef(rowIndex+2, col_im1+1) = m_JLneg(2,1)*m_hInv;
      // m_J.coeffRef(rowIndex+2, col_im1+2) = m_JLneg(2,2)*m_hInv;
      // m_J.coeffRef(rowIndex+2, col_i)     = (      -m_JRpos(2,0) + m_JLpos(2,0)-m_JRneg(2,0))*m_hInv;
      // m_J.coeffRef(rowIndex+2, col_i+1)   = (coeff*-m_JRpos(2,1) + m_JLpos(2,1)-m_JRneg(2,1))*m_hInv;
      // m_J.coeffRef(rowIndex+2, col_i+2)   = (      -m_JRpos(2,2) + m_JLpos(2,2)-m_JRneg(2,2))*m_hInv;
    }
  }
};
