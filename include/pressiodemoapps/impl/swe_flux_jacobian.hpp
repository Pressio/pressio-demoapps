...
void sweRusanovFluxJacobianThreeDof(T & JL, T & JR,
				  const T2 & qL,
				  const T2 & qR,
				  const normal_t & n,
				  const sc_t g)
...
//  F   : the flux out of the left cell (into the right cell)
  ...
  sc_t uL = qL(1)/(hL + es);
  sc_t vL = qL(2)/(hL + es);
  sc_t unL = uL*n[0] + vL*n[1];

  sc_t hR = qR(0);
  sc_t uR = qR(1)/(hR + es);
  sc_t vR = qR(2)/(hR + es);
  sc_t unR = uR*n[0] + vR*n[1];

  // rho average
  sc_t hm = 0.5*(hL + hR);
  sc_t um = (unL*std::pow(hL,0.5) + unR*std::pow(hR,0.5) )/( std::pow(hL,0.5) + std::pow(hR,0.5)  + es);
  // eigenvalues
  sc_t smax = abs(um) + abs(std::pow(g*hm, 0.5));


  sc_t termL = (n[0]*qL(1) + n[1]*qL(2)) / std::pow(qL(0),2.);

  sc_t termR = (n[0]*qR(1) + n[1]*qR(2)) / std::pow(qR(0),2.);

  sc_t hL_sqrt = std::pow(hL,0.5);
  sc_t hR_sqrt = std::pow(hR,0.5);

  sc_t hsqrt_un = hL_sqrt*unL + hR_sqrt*unR + es;
  sc_t dsmaxL[3];
  sc_t dsmaxR[3];
