			  ...
			  const sc_t gamma)
{
  ...
  const auto pL = (gamma-1)*( qL(4) - 0.5*rL*(uL*uL+vL*vL+wL*wL) );
  ...
  const auto pR = (gamma-1)*( qR(4) - 0.5*rR*(uR*uR+vR*vR+wR*wR) );
  ...
  const auto H = ( HL+RT* HR)/(1.+RT);
  const auto a = std::sqrt( (gamma-1.)*(H - 0.5*(u*u + v*v + w*w)) );
  ...
  F(2) = 0.5*( FL[2] + FR[2] + smax*(qL(2) - qR(2)) );
  F(3) = 0.5*( FL[3] + FR[3] + smax*(qL(3) - qR(3)) );
  F(4) = 0.5*( FL[4] + FR[4] + smax*(qL(4) - qR(4)) );
}
