...
public:
  ...
  state_type initialCondition() const
  {
    ...
    switch(m_probEn)
      {
      case pressiodemoapps::euler3dproblemsEnum::PeriodicSmooth:{
	euler3dsmoothInitialCondition(IC, m_meshObj, m_gamma);
	return IC;
      }
      case pressiodemoapps::euler3dproblemsEnum::SedovSymmetry:{
		...
		velocity_type & V) const
  {
    ...
    fillGhosts(state);
    velocityCellsNearBdImpl(state, timeValue, V, FL, FR, FB, FF, FD, FU,
			    uMinusHalfNeg, uMinusHalfPos,
			    uPlusHalfNeg,  uPlusHalfPos);
    velocityInnerCellsImpl(state, timeValue, V, FL, FR, FB, FF, FD, FU,
			   uMinusHalfNeg, uMinusHalfPos,
			   uPlusHalfNeg,  uPlusHalfPos);
  }
...
private:
			      ...
			      edge_rec_t & uPlusHalfPos) const
  {
    ...
    rec_fnct_t ReconstructorZ(3, m_recEn, U, m_meshObj,
			      uMinusHalfNeg, uMinusHalfPos,
			      uPlusHalfNeg,  uPlusHalfPos);
  ...
  {
    const auto stencilSize = reconstructionEnumToStencilSize(m_recEn);
    if (m_probEn == pressiodemoapps::euler3dproblemsEnum::SedovSymmetry)
    {
...
private:
  ...
  pressiodemoapps::euler3dproblemsEnum m_probEn;
