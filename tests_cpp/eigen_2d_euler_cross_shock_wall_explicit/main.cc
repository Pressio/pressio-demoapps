...
int main(int argc, char *argv[])
{
  ...
  auto appObj      = pda::create_cross_shock_wall_problem_eigen(meshObj, recEnum, 0.1, 10.);
  ...
  const auto Nsteps = 5.0/dt;
  auto time = 0.0;
  FomObserver<state_t> Obs("eulerCrossShock2dwall_solution.bin", 50);
